...
class GetMField(EventState):
    ...
    def __init__(self, topic, blocking=True, clear=False):
        super(GetMField, self).__init__(outcomes=['received', 'unavailable'],
                                              output_keys=['x_mfield', 'old_x_mfield'])
        self._topic = topic
    ...
    def execute(self, userdata):
        if not self._connected:
            userdata.x_mfield = None
            return 'unavailable'

        if self._sub.has_msg(self._topic) or not self._blocking:
            userdata.old_x_mfield = userdata.x_mfield
            userdata.x_mfield = self._sub.get_last_msg(self._topic.x)

            Logger.loginfo('%f' % userdata.x_mfield)
