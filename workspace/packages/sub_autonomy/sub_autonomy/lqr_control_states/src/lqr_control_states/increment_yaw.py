...
class IncrementYaw(EventState):
    '''
    Control any angle incrementation

    -- add_yaw_degree      int       add to yaw angle in degree
    ...
    def __init__(self, add_yaw_degree, max_time):
        super().__init__(outcomes=['finished', 'failed'])
        self.add_yaw = add_yaw_degree * pi / 180
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('IncrementYaw timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('IncrementYaw reached an outcome and is exiting')
