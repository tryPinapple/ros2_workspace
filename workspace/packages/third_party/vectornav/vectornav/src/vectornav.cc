  ...
  void * nodeptr, vn::protocol::uart::Packet & asyncPacket, size_t packetStartRunningIndex)
{
  ...
  // Populate ROS imu msg with filtered quaternions, filtered angular rates and filtered
  // body frame linear accelerations (no gravity)
  sensor_msgs::msg::Imu imu_msg;
  auto timestamp = node->getTimeStamp(cd);
  imu_msg.header.stamp = timestamp;
  imu_msg.header.frame_id = node->get_parameter("frame_id").as_string();
  ...
  if (asyncPacket.groups() & vn::protocol::uart::BinaryGroup::BINARYGROUP_ATTITUDE)
  {
    ...
    {
      vn::math::vec3f accel = cd.accelerationLinearBody();
      // Convert Native FRD (Forward, Right, Down) to ROS FLU (Forward, Left, Up)
      imu_msg.linear_acceleration.x = accel[0];  // ROS X (Forward) = Native X
      imu_msg.linear_acceleration.y = -accel[1]; // ROS Y (Left)    = Invert Native Y
      imu_msg.linear_acceleration.z = -accel[2]; // ROS Z (Up)      = Invert Native Z
    }
  }
// Applying covariances, see datasheet of VN100 for details
// Orientation (Row-major: X, Y, Z) 
// X/Roll and Y/Pitch have lower variance than Z/Yaw
...
// Angular Velocity (Uniform across all axes)
