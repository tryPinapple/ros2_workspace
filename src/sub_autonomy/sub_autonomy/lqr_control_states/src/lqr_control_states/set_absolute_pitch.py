...
class SetAbsolutePitch(EventState):
    '''
    Control any absolute angle

    -- set_pitch_degree      int        go to pitch angle -90 to 90
    ...
    def __init__(self, set_pitch_degree, max_time):
        super().__init__(outcomes=['finished', 'failed'])
        self.set_pitch = set_pitch_degree * pi / 180
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('SetAbsolutePitch timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('SetAbsolutePitch reached an outcome and is exiting')
