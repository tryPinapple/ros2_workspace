...
class IncrementY(EventState):
    '''
    This state allows to move the AUV on a specified distance on Y axis.

    -- add_y_distance      float32       Y distance to reach in meters

    -- max_time            float     Maximum time to travel the distance in s

    <= reached					     Y distance incrementation is reached

    <= failed                        Indicates that the state timed out

    '''

    def __init__(self, add_y_distance, max_time):
        super().__init__(outcomes=['reached', 'failed'])
        self.add_y = add_y_distance
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('IncrementY timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('IncrementY reached an outcome and is exiting')
        self.cancel_active_goals()

    def on_stop(self):
        Logger.loginfo('Behavior ended or IncrementY state was preempted')
        self.cancel_active_goals()
