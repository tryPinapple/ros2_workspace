...
'''
Created on 018.10.2020

@author: Thomas Germain
'''


class IncrementPitch(EventState):
    '''
    Control any angle incrementation

    -- add_pitch_degree    int       add to pitch angle in degree  

    -- max_time            float     maximum time to change the angle in s

    <= finished					     Angle incrementation is finished.
 
    <= failed                        Indicates that the state timed out

    '''

    def __init__(self, add_pitch_degree, max_time):
        super().__init__(outcomes=['finished', 'failed'])
        self.add_pitch = add_pitch_degree * pi / 180
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('IncrementPitch timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('IncrementPitch reached an outcome and is exiting')
