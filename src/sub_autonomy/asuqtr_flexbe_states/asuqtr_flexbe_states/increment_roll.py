...
class IncrementRoll(EventState):
    '''
    Control any angle incrementation

    -- add_pitch_degree      int     add to roll angle in degree
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('IncrementRoll timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('IncrementRoll reached an outcome and is exiting')
