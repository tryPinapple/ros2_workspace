...
class SetAbsoluteYaw(EventState):
    '''
    Control any absolute angle

    -- set_roll_degree       int        go to yaw angle -180 to 180 
    ...
    def __init__(self, set_yaw_degree, max_time):
        super().__init__(outcomes=['finished', 'failed'])
        self.set_yaw = set_yaw_degree * pi / 180
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('SetAbsoluteYaw timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('SetAbsoluteYaw reached an outcome and is exiting')
