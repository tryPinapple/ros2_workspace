...
class IncrementX(EventState):
    '''
    This state allows to move the AUV on a specified distance on X axis.

    -- add_x_distance      float32       X distance to reach in meters

    -- max_time            float     Maximum time to travel the distance in s

    <= reached					     X distance incrementation is reached

    <= failed                        Indicates that the state timed out

    '''

    def __init__(self, add_x_distance, max_time):
        super().__init__(outcomes=['reached', 'failed'])
        self.add_x = add_x_distance
    ...
    def execute(self, userdata):
        ...
        if (rospy.Time.now() - self.start_time) > self.max_time:
            Logger.logwarn('IncrementX timed out')
            return 'failed'
    ...
    def on_exit(self, userdata):
        self.fail = False
        Logger.loginfo('IncrementX reached an outcome and is exiting')
        self.cancel_active_goals()

    def on_stop(self):
        Logger.loginfo('Behavior ended or IncrementX state was preempted')
        self.cancel_active_goals()
